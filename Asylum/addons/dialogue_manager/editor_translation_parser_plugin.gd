...
func _parse_file(path: String) -> Array[PackedStringArray]:
	...
	if DialogueSettings.get_setting("export_characters_in_translation", true):
		...
		for character_name in character_names:
			...
			result.append(PackedStringArray([character_name.replace('"', '\\"'), "dialogue", ""]))
	...
	for key in dialogue.keys():
		...
		if line.translation_key == "" or line.translation_key == line.text:
			result.append(PackedStringArray([line.text.replace('"', '\\"'), "", ""]))
		else:
			result.append(PackedStringArray([line.text.replace('"', '\\"'), line.translation_key.replace('"', '\\"'), ""]))
